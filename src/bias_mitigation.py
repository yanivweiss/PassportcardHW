...
def train_weighted_model(X_train, y_train, X_test, y_test, group_col, 
                         weighting_method='balanced', model_type='xgboost', visualization=True):
    """
    ...
    weighting_method : str, optional
        Method for creating sample weights: 'balanced' or 'balanced_target'
    model_type : str, optional
        Type of model to train: 'xgboost' or 'linear'
    ...
    """
    # Create output directory if needed and if visualization is enabled
    if visualization:
    ...
    test_groups = X_test[group_col].values
    ...
    if visualization:
        try:
            ...
            # Calculate percent difference from overall RMSE for each group
            overall_rmse = metrics['overall']['rmse']
            groups = [g for g in metrics.keys() if g != 'overall']
            ...
            # Plot bar chart
            plt.bar(groups, rmse_values)
            plt.axhline(y=overall_rmse, linestyle='--', color='r', label='Overall')
            plt.xlabel('Group')
            plt.ylabel('RMSE')
            plt.title(f'RMSE by Group with {weighting_method.capitalize()} Weights')
            plt.legend()
            
            # Plot percent difference from overall
            plt.subplot(1, 2, 2)
            pct_diff = [(metrics[g]['rmse'] - overall_rmse) / overall_rmse * 100 for g in groups]
            
            plt.bar(groups, pct_diff)
            plt.axhline(y=0, linestyle='--', color='r')
            plt.xlabel('Group')
            plt.ylabel('% Difference from Overall RMSE')
            plt.title('% Difference from Overall RMSE')
            
            plt.tight_layout()
            plt.savefig(f'outputs/figures/bias_mitigation/weighted_model_{weighting_method}_performance.png')
            plt.close()
...
def adversarial_debiasing(X_train, y_train, X_test, y_test, group_col, 
    ...
    plt.tight_layout()
    plt.savefig(f'outputs/figures/bias_mitigation/adversarial_training_history.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'outputs/figures/bias_mitigation/adversarial_debiasing_performance.png')
    plt.close()
...
def post_processing_calibration(model, X_test, y_test, group_col):
    ...
    plt.tight_layout()
    plt.savefig('outputs/figures/bias_mitigation/post_processing_calibration.png')
    plt.close()
...
def fairness_constrained_optimization(X_train, y_train, X_test, y_test, group_col, 
                               fairness_constraint=0.1, visualization=True):
    """
    Train a model with fairness constraints using iterative optimization.
    ...
    fairness_constraint : float, optional
        Maximum allowed disparity in error rates across groups
    visualization : bool, optional
        Whether to create visualizations
    ...
    --------
    tuple
        (model, metrics) tuple with trained model and performance metrics
    """
    # Create output directory if needed and if visualization is enabled
    if visualization:
        os.makedirs('outputs/figures/bias_mitigation', exist_ok=True)
    ...
    # Remove group column from feature matrices
    X_train_model = X_train.drop(columns=[group_col]).copy()
    X_test_model = X_test.drop(columns=[group_col]).copy()
    ...
    # Track error disparity across iterations
    error_disparities = []
    ...
    for iteration in range(max_iterations):
        ...
        model.fit(X_train_model, y_train, sample_weight=weights)
        ...
        for group in unique_groups:
            ...
            if sum(group_mask) > 0:
                group_errors[group] = np.abs(y_train[group_mask] - train_pred[group_mask])
        
        # Calculate mean errors by group
        group_mean_errors = {g: np.mean(errs) for g, errs in group_errors.items()}
        
        # Calculate error disparity
        min_error = min(group_mean_errors.values())
        max_error = max(group_mean_errors.values())
        ...
        if iteration < max_iterations - 1:
            # Identify disadvantaged group (highest error)
            disadvantaged_group = max(group_mean_errors, key=group_mean_errors.get)
            
            # Increase weights for disadvantaged group
            disadvantaged_mask = (group_train == disadvantaged_group)
            weights[disadvantaged_mask] *= 1.2
    ...
    if visualization:
        try:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig('outputs/figures/bias_mitigation/fairness_constrained_convergence.png')
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig('outputs/figures/bias_mitigation/fairness_constrained_performance.png')
            plt.close()
        except Exception as e:
            print(f"Warning: Could not create visualizations: {e}")
...
def evaluate_bias_mitigation_methods(X, y, group_col, test_size=0.2, random_state=42):
    ...
    plt.tight_layout()
    plt.savefig('outputs/figures/bias_mitigation/methods_comparison.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('outputs/figures/bias_mitigation/fairness_comparison.png')
    plt.close()
...
def generate_bias_mitigation_report(evaluation_results, output_path='outputs/tables/bias_mitigation_report.md'):
    """
    ...
    report.append("![Methods Comparison](../outputs/figures/bias_mitigation/methods_comparison.png)\n")
    
    report.append("### Fairness Metrics\n")
    report.append("![Fairness Comparison](../outputs/figures/bias_mitigation/fairness_comparison.png)\n")
    
    # Add methods details
    report.append("\n## Method Details\n")
    
    report.append("### Sample Weighting\n")
    report.append("![Weighted Model Performance](../outputs/figures/bias_mitigation/weighted_model_balanced_performance.png)\n")
    ...
    report.append("![Adversarial Training History](../outputs/figures/bias_mitigation/adversarial_training_history.png)\n")
    report.append("![Adversarial Performance](../outputs/figures/bias_mitigation/adversarial_debiasing_performance.png)\n")
    ...
    report.append("![Fairness Convergence](../outputs/figures/bias_mitigation/fairness_constrained_convergence.png)\n")
    report.append("![Fairness Performance](../outputs/figures/bias_mitigation/fairness_constrained_performance.png)\n")
    ...
    report.append("![Post-Processing Calibration](../outputs/figures/bias_mitigation/post_processing_calibration.png)\n")
